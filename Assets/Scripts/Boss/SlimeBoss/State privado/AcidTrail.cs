...
public class AcidTrail : MonoBehaviour
{
    ...
    private void OnTriggerStay2D(Collider2D other)
    {
        ...
        {
            timer = 0f;
            int damage = Mathf.CeilToInt(damagePerSecond);  // redondea el daño
            health.TakeDamage(damage, transform.position); // usa el método existente
            Debug.Log($"[ACID] Dañando al jugador con {damage} desde {transform.position}");
        }
    }
